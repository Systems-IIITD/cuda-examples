...
__global__ void matmul2D(half *A, half *B, float *C) {
	...
	__shared__ half tmpA[2][2][16][16];
	__shared__ half tmpB[2][2][16][16];
	...
	int x = (blockIdx.x * 32 + wx * 16); // column
	int y = (blockIdx.y * 32 + wy * 16); // row
	int k;
	...
	row = (y + i_A);
	col = j_A;
	*((float2*)&tmpA[0][wy][i_A][j_A]) = *((float2*)&A[row * ACOL + col]);
	...
	row = i_B;
	col = x + j_B;
	*((float2*)&tmpB[0][wx][i_B][j_B]) = *((float2*)&B[row * BCOL + col]);
	...
	for (k = 0; k < ACOL; k += 16) {
		...
		if (k + 16 < ACOL) {
			...
			row = (y + i_A);
			col = k + 16 + j_A;
			*((float2*)&tmpA[write_idx][wy][i_A][j_A]) = *((float2*)&A[row * ACOL + col]);

			row = k + 16 + i_B;
			col = x + j_B;
			*((float2*)&tmpB[write_idx][wx][i_B][j_B]) = *((float2*)&B[row * BCOL + col]);
...
int main() {
  ...
  cudaMemcpy(b2d_c, b2d, Bsize, cudaMemcpyHostToDevice);
	...
	dim3 n_blocks2d;
	dim3 n_threads2d(32*4, 1);

	// c2d[y][x]; y -> row, x -> column

	n_blocks2d.x = CEIL_DIV(CCOL, 16*2); // column
	n_blocks2d.y = CEIL_DIV(CROW, 16*2); // row
