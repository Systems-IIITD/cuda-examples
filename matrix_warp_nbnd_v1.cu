...
#define AROW ((THREADS_PER_BLOCK * (25/SCALE)))
#define ACOL ((THREADS_PER_BLOCK * (30/SCALE)))
#define BROW ACOL
#define BCOL ((THREADS_PER_BLOCK * (20/SCALE))) 
#define CROW AROW
#define CCOL BCOL
...
__global__ void matmul2D(half *A, half *B, float *C) {
	...
	int x = (blockIdx.x * 64) + (threadIdx.y * 16); // column
